...
- The bash command-safety hook now auto-approves shell conditional tests (`[ … ]` POSIX test and `[[ … ]]` bash conditional). Previously only `test` was allowlisted, so compound commands using bracket tests — e.g. `while read t; do [ -f "$t" ] && …; done` — fell through to a manual permission prompt for an operation that has no side effects. In addition, `splitOnOperators` now tracks `[[ … ]]` conditional-keyword depth and no longer splits on `&&`/`||`/`|`/`;` that appear *inside* a `[[ … ]]` construct, so a conditional like `[[ -n "$x" && -f "$t" ]]` is treated as a single sub-command instead of two broken fragments. The sandbox template gains `Bash([ *)` and `Bash([[ *)` allowlist entries; single-bracket chaining (`[ a ] && [ b ]`) and operators outside `[[ … ]]` still split as before. Malformed input now fails closed: when a command does not parse to a balanced end-state (an unterminated quote, subshell, backtick, or `[[`), it is re-split so a denied command in the unparsed tail cannot hide behind the malformation and be absorbed into a single allow-matched segment.
